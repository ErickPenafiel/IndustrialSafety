...
@section('content_header')
    <h1>Lista de Trabajadores Capacitados</h1>
@stop
@section('content')
    ...
    @if (session()->has('listaCapacitacion') && count($lista) > 0 && $lista != null)
        <div class="alert alert-success">
            <strong>Se ha agregado trabajadores a la lista de capacitacion {{ count($lista) }}</strong>
        </div>
    @endif
    ...
    @if ($lista != null)
        ...
        <div class="p-2 d-flex justify-content-between align-items-center">
            <a href="{{ route('detalleCapacitacion.registrarTrabajadores', $capacitacion->id) }}"
                class="btn btn-success mt-2">Registrar Trabajadores
                Trabajadores</a>
